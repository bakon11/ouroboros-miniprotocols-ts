...
export function isILocalTxSubmitSubmit(
  stuff: any
): stuff is ILocalTxSubmitSubmit {
  return (
    isObject(stuff) && isObject(stuff.tx) && stuff.tx instanceof Uint8Array
  );
}
  ...
  implements ToCbor, ToCborObj, ILocalTxSubmitSubmit
{
  ...
  toCborObj() {
    return new CborArray([new CborUInt(0), new CborBytes(this.tx)]);
  }
